...
protocol APIClientProtocol {
    ...
    // Method to use if we expect an object response
...
final class APIClient: APIClientProtocol {
    ...
    let baseURLString: String = "https://api.jsonbin.io/v3/"
    ...
    func get<T>(endpoint: EndpointProtocol) async throws -> T where T : Codable {
        ...
        let responseData: T = try handleResponse(data: data, response: response)
    ...
    private func handleResponse<T: Codable>(data: Data, response: URLResponse) throws -> T {
        ...
        do {
            ...
            // response already in snake case
            // decoder.keyDecodingStrategy = .convertFromSnakeCase
            
            let codableData = try decoder.decode(T.self, from: data)
            return codableData
        } catch {
            debugPrint(error)
